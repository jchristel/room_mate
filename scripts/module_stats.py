...
MODULES = {
    ...
    "reports_api": ["src/reports_api.rs"],
    ...
    # The viewer, React since 2026-09-20. `static/index.html` is generated by
    # its Vite build now, so counting that file would measure the build rather
    # than the page.
    "p_index": sorted(glob.glob("src-js/viewer/*.ts"))
    + sorted(glob.glob("src-js/viewer/app/*.ts"))
