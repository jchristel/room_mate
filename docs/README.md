...
| [Plan: generalisation](PLAN-generalisation.md) | **R3 done; R1, R2 and R4 are prerequisites for doors, not suggestions.** Four seams that grew room-shaped because rooms were the only primary entity. **Read "The line in the sand" before starting doors** — R2 must land before the `Door` contract is final, because its open question (does an instance property shadow a type property?) is a contract decision, not a refactor detail |
